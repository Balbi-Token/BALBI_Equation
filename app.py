# app.py
# Calculadora da Equação de Balbi - COM BOTÃO CALCULAR
...
def calcular():
    st.session_state.calc_rodado = True
...
with st.sidebar:
    ...
    if tipo == "Circular":
        diametro = st.number_input("Diâmetro Interno (mm)", value=500, key="diametro")
        largura = altura = None
    else:
        col1, col2 = st.columns(2)
        with col1:
            largura = st.number_input("Largura (mm)", value=500, key="largura")
        with col2:
            altura = st.number_input("Altura (mm)", value=300, key="altura")
        diametro = None
    
    comprimento = st.number_input("Comprimento do Duto L (m)", value=2.0, key="comprimento")
    vazao = st.number_input("Vazão Q (m³/h)", value=2100, key="vazao")
    
    st.markdown("---")
    st.markdown("### 🔧 Métodos Empíricos (Comparação)")
    
    equal_friction = st.number_input("Equal Friction - ΔP/L fixo (Pa/m)", value=0.5, key="equal_friction")
    static_v0 = st.number_input("Static Regain - Velocidade anterior v₀ (m/s)", value=4.0, key="static_v0")
    static_eta = st.slider("Static Regain - Eficiência η (%)", 50, 95, 75, key="static_eta")
    ...
    nu = st.number_input("Viscosidade Cinemática ν (m²/s)", value=0.0000150, format="%.7f", key="nu")
    mu = st.number_input("Viscosidade Dinâmica μ (Pa·s)", value=0.0000181, format="%.7f", key="mu")
    rho = st.number_input("Massa Específica ρ (kg/m³)", value=1.20, format="%.2f", key="rho")
    relacao_vmax = st.number_input("Fator v_max / v_m (turbulento)", value=1.08474, format="%.5f", key="relacao_vmax")
...
if "calc_rodado" not in st.session_state:
    st.session_state.calc_rodado = True  # Já calcula na primeira execução

# ==================== CÁLCULOS ====================

if st.session_state.calc_rodado:
